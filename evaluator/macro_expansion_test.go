...
func Test_DefineMacros(t *testing.T) {
	input := `
let number = 1;
...
`
	...
	if macro.Parameters[0].String() != "x" {
		t.Fatalf("Parameters[0] is not 'x'. got=%q", macro.Parameters[0].String())
	}

	if macro.Parameters[1].String() != "y" {
		t.Fatalf("Parameters[0] is not 'y'. got=%q", macro.Parameters[1].String())
	}
